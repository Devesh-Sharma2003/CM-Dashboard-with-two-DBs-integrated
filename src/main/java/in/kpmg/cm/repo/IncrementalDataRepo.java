...
public interface IncrementalDataRepo extends JpaRepository <AsritCaseModel, String> {
	...
	@Query(value="SELECT \r\n"
			+ "    ah.hosp_id as hospId, \r\n"
			+ "    ah.hosp_name hospName, ah.hosp_disp_code hsCode,\r\n"
			+ "case when ah.hosp_type = 'G' then 'Government' else 'Corporate' end as hospType,\r\n"
			+ "    ah.hosp_addr1 || ' ' || ah.hosp_addr2 ||' ' || ah.hosp_addr3 as hospitalAddress,\r\n"
			+ "    al.lgd_code as stateId,\r\n"
			+ "    al.loc_name as stateName,\r\n"
			+ "    DIS.LGD_CODE AS districtCode, \r\n"
			+ "    DIS.LOC_NAME AS districtName,\r\n"
			+ "    mand.lgd_code AS hospitalMandalCode,\r\n"
			+ "    mand.loc_name AS hospitalMandal,\r\n"
			+ "    aeh.ge_code_latitude as geCodeLatitude,aeh.ge_code_longitude as geCodeLongitude,\r\n"
			+ "--    ah.cug_no mithra_contact,\r\n"
			+ "    (select max(au.cug)\r\n"
			+ "from asrim_users au \r\n"
			+ "left join asrim_mit_users anu on au.user_id = anu.user_id\r\n"
			+ "where anu.hosp_id =ah.hosp_id and user_role = 'CD10'  AND au.ACTIVE_YN = 'Y'\r\n"
			+ "and eff_end_dt is null) mithraContact ,\r\n"
			+ "    ah.hosp_contact_no as hospContactNo,    (SELECT LISTAGG(ash.speciality_id, ', ') WITHIN GROUP (ORDER BY ash.speciality_id)\r\n"
			+ "     FROM asrim_hosp_speciality ash \r\n"
			+ "     WHERE ash.hosp_id = ah.hosp_id\r\n"
			+ "         AND ash.phase_id = 6\r\n"
			+ "       and ash.renewal = 10\r\n"
			+ "       AND ash.is_active_flg = 'Y') AS specialties\r\n"
			+ "FROM \r\n"
			+ "    asrim_hospitals ah\r\n"
			+ "left join asrit_empnl_hospinfo aeh on ah.HOSP_EMPNL_REF_NUM = aeh.hospinfo_id\r\n"
			+ "LEFT JOIN asrim_locations DIS ON ah.dist_id = DIS.LOC_ID AND DIS.loc_hdr_id = 'LH6'\r\n"
			+ "LEFT JOIN asrim_locations mand ON mand.loc_id = aeh.mandal\r\n"
			+ "LEFT JOIN asrim_locations al on al.loc_id = dis.loc_parnt_id\r\n"
			+ "where ah.hosp_active_yn = 'Y' and ah.ISACTIVE_AP = 'Y'", nativeQuery = true)
